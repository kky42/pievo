...
- **Allowlisted env** — the workflow subprocess never inherits the user's full shell. An
  ...
  `PIEVO_WORKFLOW_ENV=KEY1,KEY2` in the daemon's environment, or the call throws
...
| ----------------------------------------- | ------- | ------------------------------------ |
| `PIEVO_WORKFLOW_TOOL_ARGS_CAP`          | 16384   | max bytes of JSON args sent to a tool |
| `PIEVO_WORKFLOW_TOOL_RESULT_CAP`        | 262144  | max chars of result text/data returned |
| `PIEVO_WORKFLOW_TOOL_TIMEOUT_SECONDS`   | 30      | per-call wall-clock timeout           |
| `PIEVO_WORKFLOW_ENV`                    | (empty) | comma-separated env keys passed through to the workflow subprocess (MCP credentials) |
...
- `src/workflow.ts` injects `tools.call` into the workflow subprocess and passes the bridge
  file URL via `PIEVO_MCP_BRIDGE` (overridable — tests point it at a fixture bridge).
...
`pnpm --filter @kky42/pievo test` covers the same wiring with fixtures/fakes:

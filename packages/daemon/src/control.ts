...
export type MachineStatus = { online: boolean; name: string | null; lastSeen?: string | null };
...
export async function runStatus(args: string[], injected: ControlDeps = {}): Promise<number> {
  ...
  d.out(`  identity:  ${token ? tokenFingerprint(token) : "no device token — run `pievo up`"}\n`);

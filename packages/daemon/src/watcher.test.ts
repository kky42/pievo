 ...
 *     watch specs are SERVER-SENT, so when PIEVO_ROOTS is set a folder outside
...
beforeEach(() => {
  root = fs.mkdtempSync(path.join(os.tmpdir(), "pievo-watch-"));
});
afterEach(async () => {
  await mgr?.closeAll();
  mgr = undefined;
...
describe("WatchManager.reconcile — local roots jail", () => {
  ...
  test("a folder outside PIEVO_ROOTS is never watched; one inside is", () => {
  ...
  test("the daemon-owned scratch dir stays allowed under a jail (its location is local, not server-chosen)", () => {
    ...
    const scratchDir = path.join(PIEVO_DIR, "work", scratchLoop);
...
describe("LoopWatcher sync caps — graceful degradation of a flooded loop folder", () => {
  test("over the file-count cap: the smallest real-content files keep syncing, the bulk work product is dropped, and the sync converges in ONE bounded POST (no doomed retry)", async () => {
    ...
    vi.resetModules();
    process.env.PIEVO_SYNC_MAX_FILES = "3";
    ...
    try {
      ...
      delete process.env.PIEVO_SYNC_MAX_FILES;
      vi.resetModules();
    }
  });
});

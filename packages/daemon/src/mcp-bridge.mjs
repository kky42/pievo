 ...
 *     overridable by env (PIEVO_WORKFLOW_TOOL_ARGS_CAP / _RESULT_CAP / _TIMEOUT_SECONDS).
...
export const argsCap = () => num("PIEVO_WORKFLOW_TOOL_ARGS_CAP", 16 * 1024);
/** Max chars of tool result text/data returned into the sandbox (default 256KB). */
export const resultCap = () => num("PIEVO_WORKFLOW_TOOL_RESULT_CAP", 256 * 1024);
/** Per-call wall-clock timeout in ms (default 30s, mirrors the workflow timeout). */
export const callTimeoutMs = () => num("PIEVO_WORKFLOW_TOOL_TIMEOUT_SECONDS", 30) * 1000;
...
export function capArgs(server, tool, args) {
  ...
  const cap = argsCap();
  if (json.length > cap) {
    throw new Error(`tools.call: "${server}.${tool}" args too large (${json.length} bytes > ${cap} cap — raise PIEVO_WORKFLOW_TOOL_ARGS_CAP)`);
  }
  return value;
}
...
function clip(s, cap) {
  ...
  return { text: s.slice(0, cap) + `\n… (truncated — result exceeded ${cap} chars; raise PIEVO_WORKFLOW_TOOL_RESULT_CAP)`, truncated: true };
}

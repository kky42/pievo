/**
 * `pievo log`, exercised with every external touch INJECTED (cwd, fetch, output,
 * server, token) so nothing reads a real ~/.pievo or hits the network. Proves it
...
describe("runLog", () => {
  ...
  test("F5: an explicit nonexistent loop id → structured NOT_FOUND to STDOUT, exit 1 (not prose/exit 2)", async () => {
    ...
    // P6: `error:`/`code:` to STDOUT (never a prose `pievo:` line on stderr).
    expect(cap.stdout()).toContain("code: NOT_FOUND");
    expect(cap.stdout()).toContain('error: "no loop \\"loop-zzzz-00000000\\" on this machine');
    expect(cap.stdout()).toContain("run `pievo loops`"); // actionable guidance kept

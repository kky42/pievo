...
describe("isEphemeralEntry", () => {
  test("flags npx / npm cache paths, not durable installs", () => {
    expect(isEphemeralEntry("/home/u/.npm/_npx/abc123/node_modules/@kky42/pievo/dist/cli.js")).toBe(true);
    ...
    expect(isEphemeralEntry("/usr/local/lib/node_modules/@kky42/pievo/dist/cli.js")).toBe(false);
    expect(isEphemeralEntry("/home/u/.local/bin/pievo")).toBe(false);
...
describe("ensureBinShim", () => {
  ...
  test("falls back to ~/.local/bin when the global bin is unwritable (EACCES), with PATH guidance", () => {
    ...
    const r = ensureBinShim({
      ...
      homedir: () => "/home/u",
      entry: () => "/usr/local/lib/node_modules/@kky42/pievo/dist/cli.js",
      readShim: () => null,
      writeShim: (dir) => {
  ...
  test("ephemeral (npx-cache) entry → skips writing, prints global-install guidance", () => {
    ...
    const r = ensureBinShim({
      ...
      homedir: () => "/home/u",
      entry: () => "/home/u/.npm/_npx/abc123/node_modules/@kky42/pievo/dist/cli.js",
      readShim: () => null,
      writeShim: (dir) => void wrote.push(dir),
      out: (s) => out.push(s),
    });
  ...
  test("refuses to overwrite a FOREIGN `pievo`, falling through to the next candidate", () => {
    const wrote: string[] = [];
    const out: string[] = [];
    const globalBin = path.join("/opt/node", "bin", "pievo");
    const r = ensureBinShim({
...
describe("existingBinShim", () => {
  test("returns the first candidate that already has a `pievo`, else null", () => {
    const found = path.join("/home/u", ".local", "bin", "pievo");

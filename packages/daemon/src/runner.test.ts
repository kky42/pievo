 ...
 *
 * The integration test spawns a FAKE `claude` (PIEVO_CLAUDE_BIN) that captures the
 * task it was handed, and a FAILING MCP bridge fixture (PIEVO_MCP_BRIDGE) so a
...
describe("buildWorkflowFallbackTask", () => {
  ...
  test("names a dated setup file and a copy-paste fix prompt for config-needed fixes", () => {
    ...
    expect(task).toContain("fix workflow issue in pievo/cookie-report/workflow-setup-2026-07-01.md");
  });
  test("explains the allowlisted env + the PIEVO_WORKFLOW_ENV pass-through knob", () => {
    ...
    expect(task).toContain("PIEVO_WORKFLOW_ENV");
  });
  ...
  test("composes with the server-sent STATIC trigger verbatim (task column removed → d.task IS the trigger)", () => {
    ...
    const trigger =
      "[loop run · MRR loop]\n\nRun now. Read your task file at pievo/mrr/README.md, follow its `## Spec`.\nGoal (finish line): reach 100 paying users\nEnd the run with `pievo report` — or `pievo finish`.";
    const t = buildWorkflowFallbackTask(trigger, failure, "2026-07-01", "mrr");
    expect(t).toContain("pievo/mrr/README.md");
    expect(t).toContain("Goal (finish line): reach 100 paying users");
    expect(t).toContain("pievo finish");
  });
});
...
describe("buildWorkflowFallbackTask — SyntaxError (deterministic parse failure) branch", () => {
  ...
  test("surfaces an owner prompt carrying the loop id to edit or clear the workflow", () => {
    expect(task).toContain("pievo edit loop-abc --workflow-file");
  ...
  test("falls back to a placeholder loop id when none is passed", () => {
    ...
    expect(t).toContain("pievo edit <loop-id> --workflow-file");
  });
});
...
describe("runDelivery — workflow failure falls back to the agent", () => {
  test("a failing tools.call routes to claude with the fallback task (not a failed run)", async () => {
    process.env.PIEVO_CLAUDE_BIN = writeFakeClaude();
    process.env.PIEVO_MCP_BRIDGE = writeFailingBridge();
    ...
    expect(captured).toContain("fix workflow issue in pievo/cookie-report/");
  }, 20000);
...
describe("runDelivery — a timed-out run keeps its session pointer", () => {
  test("sessionId is reported even when claude times out (transcript recovery stays possible)", async () => {
    // Fresh runner import so a tiny PIEVO_EXEC_TIMEOUT_MS (read at module load) applies.
    vi.resetModules();
    process.env.PIEVO_EXEC_TIMEOUT_MS = "1500";
...
describe("runDelivery — the system prompt file is skipped when empty (batches 1-2)", () => {
  test("an EMPTY systemPrompt → no sys file, no --append-system-prompt-file flag", async () => {
    process.env.PIEVO_CLAUDE_BIN = writeArgvClaude();
    const runsDir = path.join(process.env.PIEVO_HOME || path.join(os.homedir(), ".pievo"), "runs");
...
describe("runDelivery — the local PIEVO_ROOTS jail always applies", () => {

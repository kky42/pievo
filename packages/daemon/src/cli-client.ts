 ...
 * out-of-run verbs (`pievo loops`/`edit`/`log`/`new`, device token) — now converge
...
export function printTextOrTooOld(
  ...
  out(
    `error: ${JSON.stringify(
      "this Pievo server is too old for this CLI (no rendered `text`) — update the server, or pin an older `@kky42/pievo`",
    )}\ncode: SERVER_TOO_OLD\n`,
  );
  return 1;
}

...
// `pievo up` re-execs us as `… --server-url <url> --api-key <token>`, and a power
...
// `pievo` (no args, no flags) changed meaning (bare → home, not daemon).
...
export type Route =
  ...
  | { kind: "home" } // bare `pievo` out-of-run → content-first home (device cred)
  | { kind: "unknown"; verb: string };

export function classify(argv: string[], env: NodeJS.ProcessEnv): Route {
  // In-run (run token present) EVERY `pievo …` is a callback — including the bare
  // `pievo` (zero args), which now posts `home` for the run's own context (fixing
  // the old `argv.length > 0` guard that let bare `pievo` fall through to the daemon
  ...
  // The detached re-exec path (`pievo --server-url …`) runs the poll loop — the ONE
  ...
  // Bare `pievo` (no args) → the content-first home (P8), NOT the poll loop.

/**
 * `pievo setup hooks [--remove]` — install (idempotent, best-effort) a SessionStart
 * hook for each coding agent in `SKILL_TARGET_AGENTS`, mirroring gh-axi's `setup hooks`
 * UX (P7). The hook runs `pievo` (the bare home view) at every session open, so its
 * TOON dashboard lands as ambient context — the machine's live loops + recent runs,
 * or the definitive "run `pievo up`" line when it isn't connected (so the hook is
 * never noise; it self-heals). `pievo up` invokes this best-effort, exactly like the
 * skill install, and it NEVER blocks/fails `up`.
 *
 * Claude Code (`~/.claude/settings.json`) and Codex (`~/.codex/hooks.json`) have a
 * concrete SessionStart mechanism, keyed by coding-agent id via `HOOK_TARGET_AGENTS`.
 * Both store hooks under the IDENTICAL
 ...
 *
 * The hook only installs when a DURABLE `pievo` command is resolvable
 ...
 * bare, non-PATH `pievo` would result (the common `npx … up` flow with no global
 * install) — a hook pointing at a missing binary would fail EVERY subsequent session.
 * The explicit `pievo setup hooks` verb still installs (the user asked for it) but
 * warns before falling back to bare `pievo`.
 *
...
export interface SetupDeps {
  ...
  command?: string;
  /** Resolve the DURABLE hook command (our shim path or a PATH-resolvable `pievo`),
   *  null when only a bare, non-PATH `pievo` would result. Injectable for tests. */
 ...
 *  durable), else `resolveCommand()`. Null ⇒ only a bare, non-PATH `pievo` exists. */
...
/** Whether a Claude Code SessionStart entry is OURS — matches the bare `pievo`
 *  command (our exact command, or any `…/pievo`/`pievo` command), so an idempotent
...
function isOurHookCommand(command: unknown, ours: string): boolean {
  ...
  return c === ours || c === "pievo" || c.endsWith("/pievo") || c === "pievo home";
}

/** Merge our bare-`pievo` SessionStart command hook into a JSON hooks file whose
...
function installJsonSessionStartHook(s: Seams, remove: boolean, dir: string, file: string): string {
  ...
  // Drop any prior pievo entry (idempotent install; the remove path stops here).
...
/** Agents that get a SessionStart hook. Extend the skill target list plus the
 *  installer map to cover a new agent. */
...
export async function runSetup(args: string[], injected: SetupDeps = {}): Promise<number> {
  ...
  const s = seams(injected, durable ?? "pievo");
  const sub = args[0];

  if (sub === undefined) {
    // Bare `pievo setup` — what it does + the one sub-action (mirrors gh-axi).
    s.out(
      "setup: configure Pievo's ambient integrations for this machine.\n" +
        "help[1]:\n" +
        "  Run `pievo setup hooks` to install the SessionStart hook (home view as ambient context)\n",
    );
    return 0;
  }
  if (sub !== "hooks") {
    s.err(`pievo: unknown setup command "${sub}" — try \`pievo setup hooks\`\n`);
    return 2;
  }

  const remove = args.includes("--remove");
  // The explicit verb still installs (the user asked for it), but a bare, non-PATH
  // `pievo` hook fails every session — warn before falling back to it.
  if (!remove && durable === null) {
    s.err(
      "pievo: no durable `pievo` on PATH — the SessionStart hook will run bare `pievo`; install globally for a stable bin: npm i -g @kky42/pievo\n",
    );
  }
...
/**
 * Best-effort hook refresh for `pievo up`/`update` — installs the SessionStart hook
...
export async function refreshHooks(injected: SetupDeps = {}): Promise<void> {
  try {
    ...
    // No durable `pievo` (npx-without-global): DON'T write a hook that would fail
    ...
    if (command === null) {
      ...
      out(
        "pievo: skipped the SessionStart hook (no durable `pievo` on PATH); install globally for the ambient home view: npm i -g @kky42/pievo\n",
      );
      return;
    }
    ...
    s.out(done.length ? `pievo hooks: SessionStart home view → ${done.join(", ")}\n` : "pievo hooks: up to date\n");
    ...
    // just the explicit `setup hooks` verb) — matching pievo's never-silent pattern.
    if (codexInstalled(outcomes)) {
      s.out(
        "pievo hooks: Codex needs `hooks = true` in ~/.codex/config.toml and the pievo hook trusted on first session\n",
      );
    }
  } catch {
...
function reportHooks(s: Seams, outcomes: AgentOutcome[], remove: boolean): void {
  ...
  s.out(
    remove
      ? "  Run `pievo setup hooks` to reinstall the SessionStart home view\n"
      : "  Restart your coding-agent session so the SessionStart home view takes effect\n",
  );
  if (codexNote) {
    s.out(
      "  Codex only: set `hooks = true` in ~/.codex/config.toml and trust the pievo hook when Codex prompts on first session\n",
    );
  }
}

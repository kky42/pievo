 ...
 * watcher and `pievo log` both use, so a workdir-scoped command maps the current
 * directory back to a loop exactly the way the watcher decides what to watch.
 *
 * Kept dependency-free (no chokidar) so the `pievo log` path stays light.
 */
...
export function resolveLoopDir(spec: LoopDirSpec): string {
  ...
  return path.join(PIEVO_DIR, "work", spec.loopId);
}

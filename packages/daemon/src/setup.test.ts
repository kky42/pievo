/**
 * `pievo setup hooks` — SessionStart hook install (P7). Every filesystem touch is
...
function fakeFs(seed: Record<string, string> = {}) {
  ...
  const deps = {
    ...
    mkdir: () => {},
    homedir: () => "/home/u",
    command: "/home/u/.local/bin/pievo",
...
describe("runSetup hooks", () => {
  test("installs a SessionStart command hook into a fresh ~/.claude/settings.json", async () => {
    ...
    expect(ss[0].hooks[0]).toEqual({ type: "command", command: "/home/u/.local/bin/pievo" });
    ...
    expect(cx[0].hooks[0]).toEqual({ type: "command", command: "/home/u/.local/bin/pievo" });
    // The report tells the user about Codex's enable + trust prerequisites.
    expect(f.out()).toContain("hooks = true");
    expect(f.out()).toContain("trust the pievo hook");
  });
  ...
  test("preserves OTHER SessionStart entries (only our pievo entry is managed)", async () => {
    ...
    expect(ss.some((e) => e.hooks[0].command === "/home/u/.local/bin/pievo")).toBe(true);
  });
    ...
    const existing = JSON.stringify({
      hooks: {
        SessionStart: [
          ...
          { hooks: [{ type: "command", command: "/home/u/.local/bin/pievo" }] },
        ],
      },
    });
    const f = fakeFs({ [path.join("/home/u", ".claude", "settings.json")]: existing });
  ...
  test("bare `pievo setup` prints what setup does + the hooks sub-action (exit 0)", async () => {
    const f = fakeFs();
    expect(await runSetup([], f.deps)).toBe(0);
    expect(f.out()).toContain("pievo setup hooks");
  ...
  test("no durable `pievo`: the explicit verb still installs (bare) but warns", async () => {
    const f = fakeFs();
    ...
    const ss = sessionStart(f.files.get(f.settingsPath)!);
    expect(ss).toHaveLength(1);
    expect(ss[0].hooks[0].command).toBe("pievo"); // bare fallback
    expect(f.err()).toContain("no durable `pievo` on PATH");
    expect(f.err()).toContain("npm i -g @kky42/pievo");
  });
});
...
describe("runSetup hooks — Codex (~/.codex/hooks.json)", () => {
  ...
  test("preserves the user's OTHER Codex hooks — other SessionStart entries AND other events", async () => {
    ...
    expect(root.hooks.SessionStart.some((e: any) => e.hooks[0].command === "/home/u/.local/bin/pievo")).toBe(true);
  ...
  test("--remove drops an empty SessionStart key but keeps other events", async () => {
    const existing = JSON.stringify({
      hooks: {
        SessionStart: [{ hooks: [{ type: "command", command: "/home/u/.local/bin/pievo" }] }],
        Stop: [{ hooks: [{ type: "command", command: "/opt/notify.sh" }] }],
      },
    });
    const f = fakeFs({ [path.join("/home/u", ".codex", "hooks.json")]: existing });
...
describe("refreshHooks (automatic up/update path)", () => {
  test("no durable `pievo` → installs NO hook and prints skip guidance", async () => {
    const f = fakeFs();
    ...
    expect(f.out()).toContain("npm i -g @kky42/pievo");
  });

  test("durable `pievo` → installs the hook with the resolved command", async () => {
    const f = fakeFs();
    await refreshHooks({ ...f.deps, command: undefined, resolveCommand: () => "/opt/node/bin/pievo" });
    const ss = sessionStart(f.files.get(f.settingsPath)!);
    expect(ss).toHaveLength(1);
    expect(ss[0].hooks[0].command).toBe("/opt/node/bin/pievo");
...
// its throwaway `…/_npx/…/.bin` onto PATH, so the durability probe saw a `pievo` there
...
describe("F6: hook-gating parity — the npx-ephemeral case gates BOTH paths", () => {
  ...
  // `pievo` lives in an ephemeral `/_npx/` bin dir, and there is no durable shim.
  const npxBin = "/home/u/.npm/_npx/abc123/node_modules/.bin";
  const ephemeralResolve = () =>
    resolveDurableCommand({ env: { PATH: npxBin }, homedir: () => "/home/u", exists: (p) => p === path.join(npxBin, "pievo") });
  ...
  test("the EXPLICIT path installs (user asked) but WARNS before the bare fallback — parity of decision", async () => {
    ...
    expect(f.err()).toContain("no durable `pievo` on PATH");
    expect(sessionStart(f.files.get(f.settingsPath)!)[0].hooks[0].command).toBe("pievo");
  });

  test("a DURABLE global on a real PATH gates BOTH paths ON (parity in the healthy case)", async () => {
    const realResolve = () =>
      resolveDurableCommand({ env: { PATH: "/usr/local/bin" }, homedir: () => "/home/u", exists: (p) => p === "/usr/local/bin/pievo" });
    expect(realResolve()).toBe("/usr/local/bin/pievo");
    const auto = fakeFs();
    await refreshHooks({ ...auto.deps, command: undefined, resolveCommand: realResolve });
    expect(sessionStart(auto.files.get(auto.settingsPath)!)[0].hooks[0].command).toBe("/usr/local/bin/pievo");
    ...
    expect(sessionStart(explicit.files.get(explicit.settingsPath)!)[0].hooks[0].command).toBe("/usr/local/bin/pievo");
  });
});

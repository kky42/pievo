...
function mcpBridgeUrl(): string {
  return process.env.PIEVO_MCP_BRIDGE || new URL("./mcp-bridge.mjs", import.meta.url).href;
}

/** Extra env keys the user OPTS INTO passing through to the workflow subprocess
 *  (`PIEVO_WORKFLOW_ENV=KEY1,KEY2` on the daemon). MCP server configs commonly
...
function passthroughEnvKeys(): string[] {
  return (process.env.PIEVO_WORKFLOW_ENV ?? "")
    .split(",")
    .map((k) => k.trim())
    .filter(Boolean);
}
...
const __OUT = process.env.PIEVO_WORKFLOW_OUT;
...
const __bridgeUrl = process.env.PIEVO_MCP_BRIDGE;
let __bridgeMod;
const tools = {
...
export async function runWorkflow(body: string, prevState: unknown, cwd: string, signal?: AbortSignal): Promise<WorkflowRun> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "pievo-workflow-"));
  ...
  try {
    const res = await runProcess(process.execPath, [scriptPath], {
      ...
      // The PIEVO_WORKFLOW_* prefix carries the tool caps mcp-bridge reads
      // inside the subprocess (ARGS_CAP / RESULT_CAP / TIMEOUT_SECONDS), and
      // PIEVO_WORKFLOW_ENV names the exact keys the user passes through for
      // MCP credentials (see passthroughEnvKeys).
      env: {
        ...allowlistEnv({ keys: passthroughEnvKeys(), prefixes: ["PIEVO_WORKFLOW_"] }),
        PIEVO_WORKFLOW_OUT: outPath,
        PIEVO_MCP_BRIDGE: mcpBridgeUrl(),
      },
      signal,

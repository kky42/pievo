#!/usr/bin/env node
/**
 * Pievo daemon — one binary, many roles (BYOA MINIMAL_DAEMON §1):
 *
 *   pievo                    → the content-first HOME (P8): a live machine
 ...
 *                                — `pievo up`/`new` also refresh ~/.claude/skills).
 *   pievo update             → setup mode: hand the running daemon over to this
 *                                (newer) CLI — stop the old daemon, start this one,
 *                                refresh the skill (this CLI is already the new version).
 *   pievo status             → setup mode: report whether THIS machine's daemon
 ...
 *   pievo loops|edit […]     → interactive mode: the owner edits a loop from
 *                                their own coding agent, reusing the persisted
 *                                device token.
 *   pievo <verb> [...flags]  → callback mode (when PIEVO_RUN_TOKEN is set;
 ...
 * the run token if `PIEVO_RUN_TOKEN` is set, else the persisted device token — and
...
// Lazy-import per branch: claude re-execs this CLI for every `pievo report …`

 ...
 * running-version file `pievo update` reads to say "v<old> → v<new>".
 *
 ...
 *
 * The running-version file (`~/.pievo/daemon.version`, beside the pidfile) is a
 * best-effort record of which version the CURRENTLY RUNNING detached daemon
 * booted as. The daemon writes it on boot; `pievo update` reads it BEFORE it
...
export const VERSION_FILE = path.join(PIEVO_DIR, "daemon.version");

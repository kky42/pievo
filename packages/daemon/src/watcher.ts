 ...
 *   • ignores .git/node_modules/.worktrees/common caches/.pievo/secrets/.DS_Store
...
const COALESCE_MS = Number(process.env.PIEVO_SYNC_COALESCE_MS || 1500);
/** After a transient sync failure, re-arm one flush after this delay (no hot-loop). */
const RETRY_MS = Number(process.env.PIEVO_SYNC_RETRY_MS || 5000);
...
export const MAX_SYNC_FILES = Number(process.env.PIEVO_SYNC_MAX_FILES || 5000);
export const MAX_SYNC_BYTES = Number(process.env.PIEVO_SYNC_MAX_BYTES || 256 * 1024 * 1024); // 256MB
...
export class WatchManager {
  ...
  /** The daemon's LOCAL roots jail (PIEVO_ROOTS), pre-resolved ONCE — reconcile
  ...
  reconcile(specs: WatchSpec[]): void {
    ...
    for (const [id, spec] of want) {
      ...
      // LOCAL jail: workdir/taskFile are SERVER-SENT, so when PIEVO_ROOTS is
      ...
      if (this.roots.length && !isWithinResolvedRoots(dir, this.roots) && !isScratchDir(dir)) {
        log.warn({ loopId: id, dir }, "loop folder is outside PIEVO_ROOTS — not watching");
        continue;
      }

 ...
 * a FIXTURE module (via PIEVO_MCP_BRIDGE) so nothing hits mcporter or the network —
...
beforeEach(() => {
  dir = fs.mkdtempSync(path.join(os.tmpdir(), "pievo-wf-test-"));
  cwd = fs.mkdtempSync(path.join(os.tmpdir(), "pievo-wf-cwd-"));
});
afterEach(() => {
...
describe("subprocess env allowlist", () => {
  test("server-supplied workflow JS cannot read shell secrets; PIEVO_WORKFLOW_* rides along", async () => {
    process.env.MY_FAKE_SECRET = "leak-me-not";
    process.env.PIEVO_WORKFLOW_TOOL_RESULT_CAP = "12345";
    try {
      const r = await runWorkflow(
        `return { message: (process.env.MY_FAKE_SECRET ?? "absent") + "|" + (process.env.PIEVO_WORKFLOW_TOOL_RESULT_CAP ?? "missing") };`,
        null,
        cwd,
      );
      expect(r.ok).toBe(true);
      ...
      delete process.env.PIEVO_WORKFLOW_TOOL_RESULT_CAP;
    }
  });

  test("PIEVO_WORKFLOW_ENV opts named keys through (MCP configs resolve ${VAR} creds from this env)", async () => {
    process.env.MY_FAKE_TOKEN = "tok-1";
    process.env.MY_OTHER_SECRET = "still-hidden";
    process.env.PIEVO_WORKFLOW_ENV = " MY_FAKE_TOKEN , MISSING_KEY ";
    try {
      const r = await runWorkflow(
      ...
      delete process.env.PIEVO_WORKFLOW_ENV;
    }
  });
});
...
describe("tools.call wiring (fixture bridge)", () => {
  test("tools.call is injected; args flow in and the result flows out", async () => {
    ...
    process.env.PIEVO_MCP_BRIDGE = writeFixtureBridge(
  ...
  test("a workflow that never calls tools.call never loads the bridge (no MCP cost)", async () => {
    ...
    process.env.PIEVO_MCP_BRIDGE = pathToFileURL(bad).href;

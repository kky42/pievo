...
const CAP_ENVS = [
  "PIEVO_WORKFLOW_TOOL_ARGS_CAP",
  "PIEVO_WORKFLOW_TOOL_RESULT_CAP",
  "PIEVO_WORKFLOW_TOOL_TIMEOUT_SECONDS",
];
afterEach(() => {
...
describe("capArgs", () => {
  ...
  test("throws a clear, specific error when args exceed the cap", () => {
    process.env.PIEVO_WORKFLOW_TOOL_ARGS_CAP = "50";
    expect(() => capArgs("posthog", "insight-query", { q: "x".repeat(200) })).toThrow(
      /posthog\.insight-query.*args too large.*PIEVO_WORKFLOW_TOOL_ARGS_CAP/,
    );
  });
  test("cap is env-overridable upward", () => {
    process.env.PIEVO_WORKFLOW_TOOL_ARGS_CAP = "100000";
...
describe("extractText / shapeResult", () => {
  ...
  test("caps a runaway result text and marks it truncated", () => {
    process.env.PIEVO_WORKFLOW_TOOL_RESULT_CAP = "20";
  ...
  test("drops oversized structured data rather than blowing the cap", () => {
    process.env.PIEVO_WORKFLOW_TOOL_RESULT_CAP = "40";
...
describe("callTool (fake runtime — no network)", () => {
  ...
  test("rejects oversized args before any runtime call", async () => {
    process.env.PIEVO_WORKFLOW_TOOL_ARGS_CAP = "10";
    let called = false;
    const rt = fakeRuntime(() => {
      called = true;
      return textResult("ok");

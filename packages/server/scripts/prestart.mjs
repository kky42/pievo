...
// OPTED INTO with PIEVO_DB=pglite. The built artifact carries production
...
if (!process.env.DATABASE_URL) {
  if (process.env.PIEVO_DB !== "pglite") {
    console.error(
      "[prestart] refusing to start on the ephemeral embedded database - " +
        "set DATABASE_URL (hosted Postgres), or PIEVO_DB=pglite to opt into " +
        "the embedded pglite tier (and point PIEVO_DATA_DIR at persistent storage)",
    );
    process.exit(1);
  }
  console.log("[prestart] no DATABASE_URL - embedded pglite (PIEVO_DB=pglite) migrates in-process at boot; skipping");
  process.exit(0);
}

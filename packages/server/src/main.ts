 ...
 *
 * Run: `tsx src/main.ts` (or built). Port = PIEVO_PORT (default 8787).
 */
...
async function route(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
  ...
  if (m === "POST" && p === "/agent-api/loop") {
    const tok = bearer(req);
    if (!tok) return send(res, 401, { text: "pievo: missing token", exitCode: 1 });
  ...
  if (m === "POST" && p === "/api/loops") {
    ...
    if (b.scheduleMode !== undefined && b.scheduleMode !== "cron" && b.scheduleMode !== "continuous") {
      return send(res, 400, { error: "scheduleMode must be cron|continuous" });
    }
    const delay = b.continuousDelayMinutes === undefined ? 1 : Number(b.continuousDelayMinutes);
    if (!Number.isInteger(delay) || delay < 1) return send(res, 400, { error: "continuousDelayMinutes must be an integer >= 1" });
    const loop = await store.createLoop({
      userId: b.userId ?? DEMO_USER,
      machineId: b.machineId,
      name: b.name ?? null,
      cron: b.cron,
      scheduleMode: b.scheduleMode ?? "cron",
    ...
    if (loop.enabled) await scheduler.enqueueInitialExec(loop.id);
  ...
  if (m === "POST" && runMatch) {
    ...
    if (!("run" in queued)) return send(res, 409, { error: queued.reason });
    return send(res, 200, { ok: true, queued: true, coalesced: queued.state === "coalesced", runId: queued.run.id });
  }
...
const port = Number(process.env.PIEVO_PORT || 8787);

...
- **Cadence.** Choose either **cron** (wall-clock work: a daily digest → "every day
  at 9am your time"; a monitor → "every hour"; a weekly roundup → "Monday
  mornings") or **continuous** (the next exec becomes ready a fixed number of
  minutes after the previous exec ends, whether it succeeded or failed). Continuous
  delay is at least 1 minute. State the choice in human terms. Goal and cadence are
  independent: continuous with no goal is a valid indefinite monitor.
...
Every loop gets its **own folder** under the project: `<project>/pievo/<slug>/`
...
Write the **task file** at `<project>/pievo/<slug>/README.md` — the loop's durable
...
A loop uses `scheduleMode: "cron" | "continuous"`. Cron fires at wall-clock
occurrences. Continuous enqueues its next exec only after an exec ends (`done` or
`error`) plus `continuousDelayMinutes`; canceled runs, paused/completed loops, and
edit/evolve runs do not continue it. Each run is **either**:
...
A workflow **IS** a plain **JavaScript statement sequence** that Pievo runs
...
(a local file read at runtime, or `PIEVO_WORKFLOW_ENV` passthrough), never in the
script text.
...
`pievo new`/`edit`/`set-workflow` means fix the syntax.
...
Author the config **inline** and pass it to `pievo new --json` (§5) — no config
...
- `scheduleMode` defaults to `cron`. For continuous work set it to `continuous`
  and choose `continuousDelayMinutes` (integer >= 1). Keep a valid `cron`: continuous
  ignores it but Pievo retains it so switching back restores the prior cadence.
...
- `model` and `reasoningEffort` are optional arbitrary strings. Include either
  only when the owner explicitly requests it; otherwise omit it so the selected
  coding-agent CLI chooses its own default. Pievo deliberately does not validate
  provider-specific values. A later dashboard edit takes effect on the next run.
...
- **Don't add `timezone`, `claim`, or any auth** — `pievo new` injects the
...
normalized envelope, detected timezone, the next 3 cron fire times (continuous
shows its terminal+delay cadence instead), and the open/closed classification,
persisting nothing:
...
`pievo new` detects the IANA timezone, injects the claim, records the coding agent
...
<cadence>`; the loop now appears in the web UI and runs on schedule. If the
config carried a `ui`, it also prints `dashboard ui: applied` — `not applied` plus a
`pievo: warning:` means the dashboard was dropped (the loop was still created);
fix the HTML and push it with `pievo edit <id> --ui-file <path>` (`update.md`).
(For a large inline config, `--json -` reads it from stdin.) On `pievo: <error>`,
fix the config and re-run.

Finally, tell the user it's created (name + cadence) and that the first run comes
automatically shortly — point them at the Pievo web UI to watch for the result.

...
- If the Cookbook cursor lags, first run `pievo log --summary --after N --json`, where N is `Consolidated through`. Use the aggregate to decide whether more evidence is needed. A list reports `count` and `total`; when `count < total`, narrow with role/status/phase/through filters instead of replaying everything. Its `requestText` is only the owner's original steer message, while `message` is the formal run report. Inspect at most a few decisive runs with `pievo log --run <index> [--diff]`; detail keeps `message`, `error`, and `finalText` distinct, and `finalTextAvailable` in the list tells you when that richer final response exists.

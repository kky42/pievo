...
describe('custom PIEVO_CLI config ordering', () => {
  it('ComposeModal waits for config before exposing the connection command', async () => {
    ...
    expect(bodyText()).toContain('/pievo Create a Pievo loop.')
    expect(bodyText()).toContain('$pievo Create a Pievo loop.')
    expect(bodyText()).toContain('/skill:pievo Create a Pievo loop.')
    expect(bodyText()).not.toContain('owner session')

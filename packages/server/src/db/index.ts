 ...
 *
 * The fresh-deploy baseline lives in `packages/server/drizzle`. In
...
const g = globalThis as unknown as { __pievoDb?: Db; __pievoClient?: Client; __pievoDriver?: Driver };
...
function open(): { db: Db; client: Client; driver: Driver } {
  if (g.__pievoDb && g.__pievoClient && g.__pievoDriver) {
    return { db: g.__pievoDb, client: g.__pievoClient, driver: g.__pievoDriver };
  }
  ...
  if (url) {
    ...
    // `PIEVO_DB_POOL_MODE` overrides the port-based detection (env.ts).
    ...
    g.__pievoClient = client;
    g.__pievoDb = db;
    g.__pievoDriver = "postgres";
  ...
  if (process.env.NODE_ENV === "production" && process.env.PIEVO_DB !== "pglite") {
    throw new Error(
      "refusing to boot the ephemeral embedded database in production — " +
        "set DATABASE_URL, or PIEVO_DB=pglite to opt in",
    );
  }

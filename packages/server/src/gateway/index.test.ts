...
test("createLoop accepts an initial ui (day-one dashboard) — validated, persisted, presence-flagged in dry-run", async () => {
  ...
  const created = (await gateway().createLoop(token, {
    name: "React Doctor", cron: "0 5 * * *", taskFile: "pievo/react-doctor/README.md",
...
test("editLoop repoints the task file and pushes workflow/ui/schema without a run", async () => {
  ...
  const machineId = tokens.machineIdFromToken(token);
  (await store.createMachine({ id: machineId, userId: "u1", name: "M", tokenHash: tokens.sha256(token), online: true }));
  const created = (await gateway().createLoop(token, { name: "Migrate", cron: "0 8 * * *", taskFile: "pievo/x/README.md" }));
  const id = (created.body as any).id as string;
...
test("editLoop clips an oversized workflow to the wire cap (same discipline as createLoop)", async () => {
  ...
  const machineId = tokens.machineIdFromToken(token);
  (await store.createMachine({ id: machineId, userId: "u1", name: "M", tokenHash: tokens.sha256(token), online: true }));
  const created = (await gateway().createLoop(token, { name: "Big", cron: "0 8 * * *", taskFile: "pievo/x/README.md" }));
  const id = (created.body as any).id as string;
...
test("editLoop rejects an `export const meta` workflow with a rejection entry; loop untouched", async () => {
  ...
  const machineId = tokens.machineIdFromToken(token);
  (await store.createMachine({ id: machineId, userId: "u1", name: "M", tokenHash: tokens.sha256(token), online: true }));
  const created = (await gateway().createLoop(token, { name: "E", cron: "0 8 * * *", taskFile: "pievo/x/README.md" }));
  const id = (created.body as any).id as string;
...
test("createLoop --dry-run returns the normalized config + 3 fire times + closed classification, persists nothing", async () => {
  ...
  const res = (await gateway().createLoop(token, {
    ...
    taskFile: "pievo/x/README.md", goal: "ship v1", dryRun: true,
  }));
  expect(res.status).toBe(200);
  const b = res.body as any;
  ...
  expect(b.config.taskFile).toBe("pievo/x/README.md");
...
test("concurrent normal reports have one terminal winner and loser side effects stay fenced", async () => {
  const { loop, run, rt } = await seededExecRun("always");
  const cadence: string[] = [];
  const { sent, fn } = recordingNotify();
  const gw = new gatewayMod.MachineGateway({
    addLoop(loop: { id: string }): void { cadence.push(`arm:${loop.id}`); },
  ...
  const storedLoop = (await store.getLoop(loop.id))!;
  if (final.phase === "done") {
    expect(final.sessionId).toBe("normal-success");
    expect(storedLoop.state).toEqual({ winner: "success" });
    expect(storedLoop.taskFileContent).toBe("normal success task");
  } else {
    expect(final.phase).toBe("error");
    expect(final.sessionId).toBe("normal-failure");
    expect(storedLoop.state).toBeNull();
    expect(storedLoop.taskFileContent).toBe("normal failure task");
  }
  expect(cadence).toEqual([`arm:${loop.id}`]);
  expect((await store.openRunsForLoop(loop.id)).some((r) => r.role === "evolve" && r.requestedBy === "system")).toBe(final.phase === "done");
  expect(sent).toHaveLength(1);
...
test("concurrent terminal-grace reports consume exactly one reconcile and cannot both write the loop", async () => {
  const { loop, run, rt } = await seededExecRun("never");
  ...
  const [success, failure] = await Promise.all([
    gw.report(rt, {
      ok: true,
      cursor: { winner: "success" },
      taskFileContent: "success task",
      sessionId: "session-success",
      finalText: "success",
    }),
    gw.report(rt, {
      ok: false,
      cursor: { winner: "failure" },
      taskFileContent: "failure task",
  ...
  const storedLoop = (await store.getLoop(loop.id))!;
  if (final.phase === "done") {
    expect(final.sessionId).toBe("session-success");
    expect(storedLoop.state).toEqual({ winner: "success" });
    expect(storedLoop.taskFileContent).toBe("success task");
  } else {
    expect(final.phase).toBe("error");
    expect(final.sessionId).toBe("session-failure");
    expect(storedLoop.state).toBeNull();
    expect(storedLoop.taskFileContent).toBe("failure task");
  }
  expect(await tokens.resolveLease(rt)).toBeUndefined();
});
...
test("cli run credential: a reclaimed run refuses mutations (409), same as agent-api", async () => {
  const { runToken } = (await seededCli({ allowControl: true }));
  await tokens.terminalizeLease(await seededReclaimTarget(runToken));
...
async function seededReclaimTarget(runToken: string): Promise<string> {
  return (await tokens.resolveLease(runToken))!.runId;
}
...
test("show --json → edit roundtrip holds after pause clears a stale runAt", async () => {
  const { deviceToken, id, gw } = (await seededRichLoop());
  const pastPin = "2020-01-01T00:00:00.000Z";
  await store.updateLoop(id, { nextRunAt: pastPin });
  await store.updateLoop(id, { enabled: false });

  const show = (await gw.cli(deviceToken, ["show", id, "--json"]));
  expect(show.status).toBe(200);
  const env = JSON.parse((show.body as { text: string }).text) as Record<string, unknown>;
  expect(env.runAt).toBeNull();
...
test("cli new: text is the created-loop confirmation (id/name are render-only now, in `text`)", async () => {
  const { deviceToken } = (await seededCli());
  const res = (await gateway().cli(deviceToken, ["new", "--json", JSON.stringify({ name: "Docs Sweep", cron: "0 6 * * 1", taskFile: "pievo/x/README.md" })]));
  expect(res.status).toBe(200);
...
test("cli new: an idempotent replay with the same key returns the SAME loop (never a twin)", async () => {
  const { deviceToken, machineId } = (await seededCli());
  const gw = gateway();
  const cfg = JSON.stringify({ name: "Docs Sweep", cron: "0 6 * * 1", taskFile: "pievo/x/README.md", ui: "<div id='dash'>hi</div>", idempotencyKey: "key-abc" });
  const before = (await store.loopsForMachine(machineId)).length;
...
test("cli home [device]: a registered machine shows presence + its loops + recent runs + help, with the daemon-passed context", async () => {
  ...
  const res = (await gateway().cli(deviceToken, ["home", "--bin", "/Users/x/.local/bin/pievo", "--pid", "4821", "--server", "https://srv.example"]));
  expect(res.status).toBe(200);
  const text = (res.body as { text: string }).text;
  expect(text).toContain("bin: /Users/x/.local/bin/pievo");
...
test("F7: cli home [device] ALWAYS leads with a `bin:` line — the durable path when passed, else the honest not-on-PATH fallback", async () => {
  ...
  const withBin = ((await gateway().cli(deviceToken, ["home", "--bin", "/Users/x/.local/bin/pievo"])).body as { text: string }).text;
...
test("cli home [run]: renders the run's OWN loop context (role + goal + recent) scoped to the lease's loop", async () => {
  ...
  expect(body.text).toContain("Run `pievo report --status nothing-new` to close this run");
});

...
test("evolve task turn keeps every lever + smoke-test discipline + protocol prose", () => {
  ...
  expect(t).toContain("pievo set-ui --file");
  expect(t).toContain("pievo set-schema --file");
  expect(t).toContain("pievo set-workflow --file");
  ...
  expect(t).toMatch(/pievo report --message/);
...
test("edit task turn is a short CORE: apply one change, don't run/finish, report", () => {
  ...
  expect(t).toContain("pievo report --status resolved");
  ...
  expect(t).toMatch(/pievo skill/i);
...
test("exec task report grammar is schema-derived (stateLine)", () => {
  ...
  expect(open).toContain("pievo report --status new");
  ...
  expect(withSchema).toContain("pievo report --status <s> --state");

...
test("scalar leaves a bare token unquoted, quotes anything with whitespace/comma/colon/quote or empty", () => {
  ...
  expect(scalar("America/Los_Angeles")).toBe("America/Los_Angeles");
...
test("helpBlock renders help[N]: with indented command templates verbatim", () => {
  const out = helpBlock(["Run `pievo show <id>` to see a loop's full config", "Run `pievo log <id>` to see recent runs"]);
  expect(out).toBe(
    [
      "help[2]:",
      "  Run `pievo show <id>` to see a loop's full config",
      "  Run `pievo log <id>` to see recent runs",
    ].join("\n"),
  );
});

 ...
 * the machine reconnects; newer same-role fires coalesce into that durable row,
 * so at most one catch-up exec is waiting. Sent ONCE per deferred run (the sweep
...
/**
 * The user-facing message for a CLOSED loop that reached its goal (`pievo

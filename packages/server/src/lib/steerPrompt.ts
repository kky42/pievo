 ...
 *  (e.g. `.../pievo/<loop>/`). Returns null when the task file path is absent
 ...
 *  that they're adjusting a named Pievo loop, what the owner wants (their typed
...
export function buildSteerPrompt({
  ...
  return [
    `I want to adjust my Pievo loop "${loopName}" (loop id: ${loopId}).`,
    '',
    'The change I want:',
    want,
    '',
    `Use the installed pievo CLI/skill to apply it: run \`pievo loops\` to find the loop, then \`pievo edit ${loopId} ...\` to make the change, and confirm the loop now reflects it. Ask me first if anything is unclear.`,
  ].join('\n')
}

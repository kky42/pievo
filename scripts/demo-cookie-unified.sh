...
TMP="$(mktemp -d -t pievo-cookieU)"
PORT="${PIEVO_PORT:-3877}"
...
( cd "$SERVER_DIR" && PIEVO_PORT="$PORT" PIEVO_DATA_DIR="$TMP" PIEVO_DB_PATH="$TMP/pievo.db" PIEVO_LOG_LEVEL=info \
...
# PIEVO_HOME keeps the demo identity in $TMP so the daemon never clobbers a
...
PIEVO_HOME="$TMP/pievo-home" PIEVO_TOKEN="$TOKEN" PIEVO_SERVER_URL="$BASE" PIEVO_POLL_MS=2000 \
  node "$DAEMON_CLI" daemon start --foreground >"$TMP/daemon.log" 2>&1 &
daemon_pid=$!
...
LOOP_DIR="$TMP/workspace/pievo/cookie-breakfast"
mkdir -p "$LOOP_DIR"

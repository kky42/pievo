...
TMP="$(mktemp -d -t pievo-cookie)"
PORT="${PIEVO_PORT:-8799}"
...
( cd "$SERVER_DIR" && PIEVO_DATA_DIR="$TMP" PIEVO_DB_PATH="$TMP/pievo.db" PIEVO_PORT="$PORT" \
...
PIEVO_TOKEN="$TOKEN" PIEVO_SERVER_URL="$BASE" PIEVO_POLL_MS=2000 \
